...
fn id(base: u64) -> u64 {
    ...
    .iter()
    .fold(0, |id_running, &r| {
        id_running.wrapping_shl(8) | (read_register(base, r) as u8 as u64)
    })
}

/// Disables the functional parts of the UART, drains FIFOs,
/// sets baud rate and enables the UART in the polling mode.
fn reset_and_init(base: u64) -> u64 {
    let id = id(base);

    // Mask interrupts (lower 11 bits)
    write_register(base, Pl011Register::Imsc, 0x7ff);
    // Clear interrupts (lower 11 bits)
    write_register(base, Pl011Register::Icr, 0x7ff);
    // Disable DMA on Rx and Tx
    write_register(base, Pl011Register::DmaCr, 0x0);

    // Leave Rx and Tx enabled to drain FIFOs.
    write_register(base, Pl011Register::Cr, CR_RX_ENABLE | CR_TX_ENABLE);
    read_register(base, Pl011Register::Cr); // wait
    read_register(base, Pl011Register::Cr); // wait
    poll_not_busy(base);

    // Disable Rx, Tx, and UART.
    write_register(base, Pl011Register::Cr, 0x00000000);

    // Set integer and fractional parts of the baud rate,
    // harcoded for now
    write_register(base, Pl011Register::Fbrd, 0x00000004);
    write_register(base, Pl011Register::Ibrd, 0x00000027);
    ...
    // UARTLCR_H write
    write_register(base, Pl011Register::LcrHigh, LCR_H_FIFO_EN | LCR_H_8BITS);

    // Clear the errors
    write_register(base, Pl011Register::RsrOrEcr, 0);
...
fn read_register(base: u64, reg: Pl011Register) -> u32 {
    unsafe { core::ptr::read_volatile((base + reg as u64) as *const u32) }
}

fn write_register(base: u64, reg: Pl011Register, val: u32) {
    unsafe {
        core::ptr::write_volatile((base + reg as u64) as *mut u32, val);
    }
}

fn poll_tx_not_full(base: u64) {
    while read_register(base, Pl011Register::Fr) & FR_TX_FULL != 0 {
...
fn poll_not_busy(base: u64) {
    while read_register(base, Pl011Register::Fr) & FR_BUSY != 0 {
        #[cfg(target_arch = "aarch64")]
        unsafe {
            core::arch::asm!("yield");
        }
...
impl Pl011 {
    pub fn reset_and_init(&self) -> u64 {
        reset_and_init(PL011_BASE)
    }

    pub fn send_byte(&self, byte: u8) {
        poll_tx_not_full(PL011_BASE);
        write_register(PL011_BASE, Pl011Register::Dr, byte.into());
    }
}
